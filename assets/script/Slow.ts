...
export default class SLow extends cc.Component {
    ...
    is_pause: boolean = false; // for search_light.ts
    ...
    @property(cc.Prefab)
    time_pause_prefab: cc.Prefab = null;
    ...
    click_callback(){ //按下技能按鈕時的事件
        ...
        /// gen text
        this.scheduleOnce(this.gen_time_pause_prefab, 0);
        ...
        this.scheduleOnce(() =>{ // skill end
            cc.find("Canvas/background").color = new cc.Color(255, 255, 255, 255);
            cc.find("Canvas/Main Camera/time_pause").destroy();
            this.no_detection = false;
            this.is_pause = false;
        }, 5);
        /// gen text
        
        cc.find("Canvas/background").color = new cc.Color(124, 231, 146, 168);
    ...
    gen_time_pause_prefab(){
        var time_pause = cc.instantiate(this.time_pause_prefab);
        time_pause.setPosition(0, 200);
        cc.find("Canvas/Main Camera").addChild(time_pause);
    }
}
